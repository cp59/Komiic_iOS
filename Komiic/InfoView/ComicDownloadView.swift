                    ...
                    List {
                        ...
                        Section {
                            ForEach(Array(chaptersList.enumerated()),id: \.element.id) { index,chapter in
                                let downloaded = FileManager.default.fileExists(atPath: comicDataPath.appendingPathComponent(chapter.id).path)
                                if (chapter.type == "book") {
                                    Button(chapter.serial+"卷") {
                                        selectedChapter = chapter
                                        startDownload = true
                                    }.badge(downloaded ? "已下載" : "\(chapter.size)p").disabled(downloaded)
                            ...
                            ForEach(Array(chaptersList.enumerated()),id: \.element.id) { index,chapter in
                                ...
                                if (chapter.type == "chapter") {
                                    Button(chapter.serial+"話") {
                                        selectedChapter = chapter
                                        startDownload = true
                                    }.badge(downloaded ? "已下載" : "\(chapter.size)p").onAppear{haveChapter = true}.disabled(downloaded)
                                }
                            }
                        } header: {
                            if (haveChapter) {
